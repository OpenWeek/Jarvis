...
def word_analyzer(corpus,number,filename,title, app_id,app_key, lang,loc='',saving=True, jarvis=None):
    ...
    while counter<=number:
        ...
        if code==200:
            if saving:
                f.write(str(idd)+'. '+word.capitalize()+ '. ' + corpus_words_dic[word][0]+'. IFD=' +str(corpus_words_dic[word][1])+'. Frequency=' +str(ferquenc)+'.\n' )
            jarvis.say(str(idd)+'. '+word.capitalize()+ '. ' + corpus_words_dic[word][0]+'. IFD=' +str(corpus_words_dic[word][1])+'. Frequency=' +str(ferquenc)+'.\n' )
    ...
    if testCheck.strip().lower()=='y':
        ...
        for i in testyourself:
            ...
            if s.ratio()>.8:
                ...
                flag=True
            if saving:
                f.write(str(ids)+'. Your answer: ' + userAsnwer[ids-1]+'| Correct Answer:'+i[2]+ '| ' + str(flag) + '\t')
            jarvis.say(str(ids)+'. Your answer: ' + userAsnwer[ids-1]+'| Correct Answer:'+i[2]+ '| ' + str(flag) + '\t')
...
def call(jarvis, s):
    ...
    """
    jarvis.say('This is an application which extracts the unique words related to a query from internet and news, then generates a .txt output file that contains the definition, etymology, example, and question about the extracted words.')
    jarvis.say('Note that you need to have an oxford api-key in order to continue the process. If you do not possess the key, please refer to https://developer.oxforddictionaries.com/ to obtain your api-key.')
    ...
    while (True):
        ...
        if savecheck.strip().lower()=='y':
            loc=str(jarvis.input('Enter the folder location in which the output ".txt file" will be stored (e.g. E:/Documents/Words/)' ))
            saving=True
        jarvis.say('Please wait to complete the process.' )
        corpus=get_text_from_news(query.lower(),apiKey)
        try:
            word_analyzer(corpus,numofwords, query.capitalize(),'News Word Analysis for ' + query , app_id,app_key ,'fr',loc,saving, jarvis)
        except:
            jarvis.say("An error appears. The inputs such as folder location might have been wrongly entered or maybe you don't have write access. ")
            continue
